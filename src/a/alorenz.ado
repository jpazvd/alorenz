*! version 3.3.1       	<20230205>       JPAzevedo
*		select option is now working
...
program define alorenz   , rclass
  ...
  * Loop over all the commands to test if they are already installed, if not, then install
	...
	if ("`select1'" != "") | ("`select2'" != "") & ("`select'" == "") {
		local select "`select1' | `select2' "
	}
    ...
    if ("`by'"!="") & (("`mark'" != "") |  ("`markvar'" != "")) {
        di as err "by() option is not supported jointly with markvar and mark"
        exit 198
    }
    ...
    quietly {
		...
		***************************
		...
		if ("`select'" != "") {
			...
			tknz "`select1'" , parse("|") s(v) nochar
			local selectcat1 = `s(items)'
			tknz "`select2'" , parse("|") s(v) nochar
			local selectcat2 = `s(items)'

			tknz "`select'" , parse("|") s(v) nochar
			local selectcat = `s(items)'
			
			
			if ( "`select1'" == "" ) {
				forvalues sel = 1(1)`selectcat' {
			...
			/* selectcat2 */
				local selectcat2 = `selectcat1'+1
				forvalues sel = `selectcat2'(1)`selectcat' {
					loc varsel = word("`v`sel''",1)
					loc criter = word("`v`sel''",2)
					loc valsel = word("`v`sel''",3)
					loc k = 2
			...
			/* JPA identify the total number of categories identified by SELECT*/
			local selectcat = `s(items)'
...
if ("`mark`k''" == "") {
	local mark`k' = real(word("`mark1'",-1))
	local labelsel`k' = word("`labelsel1'",-1)
}
...
noi list   `selname_p`s''   bvector`tmp2' bvector`tmp3' bvector`tmp4' if  bvector`tmp2'  != .
				   ...
				   if ("`invert'"=="") {                                                                                                   /* JPA 15/05/2008*/
						local sel_l`s' "(scatter `sel_l`s'' bvector`tmp1', mlabel(`selname_l`s'') mcolor(black) mlabangle(`mlabangle') mlabsize(`mlabsize') msize(`msize') mcolor(`mcolor') mfcolor(`mfcolor') mlcolor(`mlcolor'))"
						local sel_p`s' "(scatter `sel_p`s'' bvector`tmp4', mlabel(`selname_p`s'') mcolor(black) mlabangle(`mlabangle') mlabsize(`mlabsize') msize(`msize') mcolor(`mcolor') mfcolor(`mfcolor') mlcolor(`mlcolor'))"
						local sel_e`s' "(scatter `sel_e`s'' bvector`tmp4', mlabel(`selname_e`s'') mcolor(black) mlabangle(`mlabangle') mlabsize(`mlabsize') msize(`msize') mcolor(`mcolor') mfcolor(`mfcolor') mlcolor(`mlcolor'))"
					}

					if ("`invert'"!="") {                                                                                                   /* JPA 15/05/2008 */
						local sel_l`s' "(scatter bvector`tmp1'  `sel_l`s'', mlabel(`selname_l`s'') mcolor(black) mlabangle(`mlabangle') mlabsize(`mlabsize') msize(`msize') mcolor(`mcolor') mfcolor(`mfcolor') mlcolor(`mlcolor'))"
						local sel_p`s' "(scatter bvector`tmp4'  `sel_p`s'', mlabel(`selname_p`s'') mcolor(black) mlabangle(`mlabangle') mlabsize(`mlabsize') msize(`msize') mcolor(`mcolor') mfcolor(`mfcolor') mlcolor(`mlcolor'))"
						local sel_e`s' "(scatter bvector`tmp4'  `sel_e`s'', mlabel(`selname_e`s'') mcolor(black) mlabangle(`mlabangle') mlabsize(`mlabsize') msize(`msize') mcolor(`mcolor') mfcolor(`mfcolor') mlcolor(`mlcolor'))"
					}
